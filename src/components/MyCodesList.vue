        ...
        <div class="on-canvas" style="display:inline-block; margin-left: 30px; position: relative;" @mousedown="startSelectFunction">
            <div :class="'code-list'+index" v-for="item in codes_message" >
                ...
                <VisualCode ref="subcode" :target="index" :mapping="item.idx" :draw_message="item.value" 
                    :line_opacity="line_opacity" :rect_opacity="rect_opacity" :show="item.show" :code_name="item.name" :show_name="show_name" :filter="item.filter"/></div>
            </div>
        </div>
...
export default{
    ...
    methods: {
        ...
        realGetCodeRange(){
            ...
            console.log(filter_list[this.index])
